...
Usage: python scraper_test.py [wate|wcyb|wuot|tnholler|wbir] [--full] [--keyword WORD]
...
To rebuild the venv, do pip install -r requirements.txt

KNOWN ISSUE: WCBY is broken again. moving on for now and will cirlce back if time allows. 

"""
import sys
from scrapers import WATEScraper, WCYBScraper, WUOTScraper, TnHollerScraper, WBIRScraper, KnoxNewsScraper, GoogleNewsRSSScraper
...
def test_scraper(name, cls, fetch_full=False, keyword=None):
    ...
    try:
        ...
        if keyword:
            ...
            if name in ("wate", "wbir"):  # both support fetch_full
